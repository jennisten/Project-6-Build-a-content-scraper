...
//variable for defining execution of data folder and csv file
let execute;
...
scrapeIt(productRoute, {
	...
	if (`${response.statusCode}` == 200) {
		...
		//set execute to true so data folder and csv will be created
		execute = true;
		...
		//throw error if something's not right
	  	console.dir(`Something went wrong. Please try again later.(${response.statusCode})`);
	}
}).catch(function (error) {
	console.error(`Something went wrong. Cannot connect to ${mainPage}. Please try again later.`);
	execute = false;
});
